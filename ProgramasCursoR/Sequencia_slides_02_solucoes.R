...
#Cria a variável Gini
pobre.df$Gini<-cut(pobre.df$Var09,
                   c(min(pobre.df$Var09),0.2,0.4,
                     max(pobre.df$Var09)))

pobre.df$Pobre<-cut(pobre.df$Var03,
                   c(min(pobre.df$Var03),50,80,
                     max(pobre.df$Var03)))
#Tabela de frequência
CrossTable(pobre.df$Gini, pobre.df$Pobre)
